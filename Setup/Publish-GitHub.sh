...
if [[ "$BRANCH" == "master" ]]; then
    ...
    if [[ $UNCOMMITED_FILE_COUNT == 0 ]]; then
        ...
        if [[ $? == 0 ]]; then
            ...
            ASSET_UPLOAD_URL=`curl -s -H "Authorization: token $TOKEN" --data "{\"tag_name\": \"latest\", \"target_commitish\": \"master\", \"name\": \"Most recent build ($VERSION_NUMBER)\", \"body\": \"This is the most recent automated build.\", \"draft\": false, \"prerelease\": true}" -X POST https://api.github.com/repos/$OWNER/$REPOSITORY/releases | grep "\"upload_url\"" | cut -d\" -f4 | cut -d{ -f1`
            for FILE_EXTENSION in "exe" "zip"; do
